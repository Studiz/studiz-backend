...
const deleteClassroom = async (req, res, next) => {
    try {
        ...
        teacherClassroom.splice(
            teacherClassroom.findIndex((data) => data.id === id),
            1
        );
        ...
        students.forEach(async (doc) => {
            ...
            studentClassroom.splice(
                studentClassroom.findIndex((data) => data.id === id),
                1
            );
    ...
    try {
        ...
        for (let i = 0; i < students.length; i++) {
            ...
            classroomInstd.forEach((data) => {
                if (data.id == id) {
                    (data.id = id), (data.name = body.name), (data.description = body.description), (data.color = body.color);
                }
            });
            await student.update(studentData);
        }
        ...
        classroomInstd.forEach((data) => {
            if (data.id == id) {
                (data.id = id), (data.name = body.name), (data.description = body.description), (data.color = body.color), (data.relevantSubjects = body.relevantSubjects);
            }
        });
        await teacher.update(teacherData);
...
const getClassroomById = async (req, res, next) => {
    try {
        ...
        const getTeacher = await teachers.get();
        ...
        const filterData = userArray.filter((student) => {
            return uid.includes(student.uid);
        });
        const classroomInUser = filterData[0].classrooms;
        const filterClassroom = classroomInUser?.filter((classroom) => {
